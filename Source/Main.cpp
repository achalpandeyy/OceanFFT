...
struct OceanFFT final : public Ogle::Application
{
    ...
    void Initialize() override
    {
        ...
        twiddle_factors = std::make_unique<Ogle::Texture2D>(glm::log2(DISPLACEMENT_MAP_DIM), DISPLACEMENT_MAP_DIM, GL_RGBA32F, GL_RGBA,
            GL_FLOAT);

        twiddle_factors_program = std::make_unique<Ogle::Shader>("C:/Projects/OceanFFT/Source/Shaders/TwiddleFactors.comp");
        twiddle_factors_program->Bind();
        ...
        // Todo: I can't use a binding point of 0, if I do it disturbs tilde_h_k_t_d* class of textures as I can
        // see as artifacts when I visualize them, how tf is this happening?
        twiddle_factors->BindImage(6, GL_WRITE_ONLY, GL_RGBA32F);
...
private:
    ...
    std::unique_ptr<Ogle::Texture2D> twiddle_factors = nullptr;

    std::unique_ptr<Ogle::Shader> tilde_h0_program = nullptr;
    std::unique_ptr<Ogle::Shader> tilde_h_k_t_program = nullptr;

    std::unique_ptr<Ogle::Shader> twiddle_factors_program = nullptr;
